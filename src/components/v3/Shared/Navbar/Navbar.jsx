...
const Navbar = ({ pageName, posts, course }) => {
  ...
  const eventsOutRef = useRef();
  const timeoutDuration = 0;
  const handleEventsEnter = (isOpen) => {
    clearTimeout(eventsOutRef.current);
    !isOpen && eventsRef.current?.click();
  };
  const handleEventsLeave = (isOpen) => {
    eventsOutRef.current = setTimeout(() => {
      isOpen && eventsRef.current?.click();
    }, timeoutDuration);
  };
    ...
    <>
      <header
        className={`border-b border-[#EAECF0] ${
          isSticky ? "fixed top-0 z-50 w-full bg-white shadow-md " : ""
        }`}
      >
              ...
              <>
                <div className="flex gap-5 space-x-2">
                  ...
                  <Popover className="hover:border-none">
                    {({ open }) => (
                      <div
                        onMouseEnter={() => handleEventsEnter(open)}
                        onMouseLeave={() => handleEventsLeave(open)}
                      >
                        <Popover.Button
                          ref={eventsRef}
                          className={`flex items-center gap-x-1 py-3 outline-none ${
                        ...
                        <Transition
                          ...
                          leaveTo="opacity-0 translate-y-1"
                        >
                          <Popover.Panel className="absolute z-50 mt-2 border bg-white shadow-xl">
                            <Link
